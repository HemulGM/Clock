...
uses
  ...
  Vcl.Controls, Vcl.Forms, Vcl.Dialogs, Direct2D, D2D1, Vcl.ExtCtrls,
...
procedure TForm1.TimerRepaintTimer(Sender: TObject);
begin
 DoDraw;
 //FRotate:=FRotate + 5;
 //if FRotate > 360 then FRotate:=1;
 TempD:=TempD + 1;
 if TempD >= 30 then TempD:=1;
end;
...
begin
 with Canvas do
  begin
   ...
   Font.Size:=10;
   TmpStr:=IntToStr(D);
   TmpRect:=Rect(Center.X-10, Center.Y - 123, Center.X-10+20, Center.Y - 123 + 20);
